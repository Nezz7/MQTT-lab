...
func main() {
	...
	opts.SetClientID("go_mqtt_clientSM")
	opts.SetUsername("sm")
	...
	go publish(client, "/topic/consumption", 60, 2)
	go publish(client, "/topic/production", 15, 2)

	go sub(client, "/topic/price", 1)
	go sub(client, "/topic/reductionVoltage", 2)

	time.Sleep(time.Second * 60 * 15)
	client.Disconnect(250)
}

func publish(client mqtt.Client, topic string, freq float64, qos byte) {
	num := 5
	for i := 0; i < num; i++ {
